        ...
        if (size === 'small') {
            return (
                <div className="flex items-center gap-1.5 font-bold">
                    ...
                    <span className="text-text-muted text-xs text-text-muted">/ {total}</span>
                </div>
            )
        }
            ...
            <div className="space-y-6 animate-fade-in max-w-3xl mx-auto">
                ...
                <div className="glass rounded-3xl p-6 md:p-8 border border-border-main">
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div className="text-center sm:text-left space-y-2">
                            <h2 className="text-2xl font-black text-text-main">Reviewing Practice</h2>
                            ...
                            <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                                ...
                                <span className="bg-black/10 dark:bg-white/10 text-text-muted text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-border-main">{selectedSet.topic}</span>
                            ...
                            <p className="text-xs text-text-muted mt-2">{formatDate(selectedSet.created_at)}</p>
                        </div>
                    </div>
                </div>
                ...
                {isMcq ? (
                                    ...
                                    <div key={i} className={`rounded-xl p-4 border ${ans?.isCorrect ? 'bg-green-500/5 border-green-500/15' : 'bg-red-500/5 border-red-500/15'
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-medium text-text-main mb-2">Q{i + 1}. {q.question}</p>
                                                {q.diagramUrl && (
                                                    <img src={q.diagramUrl} alt="diagram" className="max-h-48 object-contain rounded-lg border border-border-main mb-3 bg-white" />
                                                )}
                                                <p className="text-xs text-text-muted bg-black/20 p-2 rounded-lg mb-2">
                                                ...
                                                <div className="text-xs text-text-muted mt-2 bg-black/5 dark:bg-white/5 p-3 rounded-lg border border-border-main">
                                                    <span className="font-bold text-text-muted">Explanation:</span> {q.explanation}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="glass rounded-3xl p-6 md:p-8 border border-border-main space-y-4">
                            <h3 className="text-lg font-bold text-text-main">Question</h3>
                            {pd.questionData?.diagramUrl && (
                                <img src={pd.questionData.diagramUrl} alt="diagram" className="w-full max-h-64 object-contain rounded-xl border border-border-main mb-4 bg-white" />
                            )}
                            <div className="text-[15px] text-text-muted whitespace-pre-wrap leading-relaxed">
                                {pd.questionData?.question}
                            </div>
                            <div className="mt-4 pt-4 border-t border-border-main">
                                <h3 className="text-sm font-bold text-text-muted uppercase tracking-wider mb-2">Your Answer</h3>
                                <div className="text-[15px] text-text-main whitespace-pre-wrap leading-relaxed italic bg-black/20 p-4 rounded-xl">
                                    {pd.answer}
                                </div>
                            </div>
                        </div>
        ...
        <div className="space-y-8 animate-fade-in max-w-5xl mx-auto pb-12">
            ...
            {!isEmbedded && (
                <div>
                    ...
                    <p className="text-text-muted text-lg">Review and learn from your previous AI practice sessions.</p>
                </div>
            )}
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20">
                    ...
                    <p className="text-text-muted font-medium">Loading saved history...</p>
                </div>
            ) : error ? (
                <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 text-center text-red-400">
                    <span className="material-symbols-outlined text-4xl mb-2">error</span>
                    <p className="font-bold">{error}</p>
                    <button onClick={fetchSavedSets} className="mt-4 px-4 py-2 bg-black/5 dark:bg-white/5 border border-border-main rounded-xl text-white hover:bg-black/10 dark:bg-white/10 transition-colors">
                        Try Again
                    </button>
                </div>
            ) : savedSets.length === 0 ? (
                <div className="glass rounded-3xl p-12 border border-border-main text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {savedSets.map(set => (
                        <div key={set.id}
                            ...
                            className="glass rounded-2xl p-6 border border-border-main hover:bg-black/5 dark:bg-white/5 hover:border-border-main transition-all cursor-pointer group flex flex-col h-full relative overflow-hidden">
                            {/* Card Glow */}
                            ...
                            <h3 className="font-bold text-text-main text-lg mb-1 line-clamp-2 relative z-10 leading-snug">{set.topic}</h3>
                            <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-4 relative z-10">{set.subject.replace('_', ' ')}</p>

                            <div className="mt-auto pt-4 border-t border-border-main flex items-center justify-between relative z-10">
