...
// ─── Polyfill DOMMatrix for Vercel serverless (text extraction only, no canvas) ─
if (typeof globalThis.DOMMatrix === 'undefined') {
    globalThis.DOMMatrix = class DOMMatrix {
        constructor(init) {
            const values = new Float64Array(16);
            values[0] = values[5] = values[10] = values[15] = 1;
            if (Array.isArray(init)) { for (let i = 0; i < init.length && i < 16; i++) values[i] = init[i]; }
...
export async function ingestPDF(fileBuffer, metadata) {
    try {
        // Use pdfjs-dist legacy build (no DOMMatrix/canvas required — Vercel safe)
        ...
        // Batch insert into Supabase (groups of 50 to avoid payload limits)
        const batchSize = 50;
        let inserted = 0;
