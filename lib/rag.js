...
/**
 * Ingest a PDF file: read, split into chunks, embed, and store in Supabase.
 * @param {string} filePath - Absolute path to the PDF file.
 * @param {object} metadata - { subject, level, year, type, filename }
 * @returns {Promise<number>} Number of chunks inserted.
 */
export async function ingestPDF(filePath, metadata) {
    try {
        // Read the PDF file
        const fileBuffer = await readFile(filePath);

        // Dynamic import to avoid pdf-parse build-time test-file issue
